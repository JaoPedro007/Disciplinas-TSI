        ...
        while (barbearia.aberta || !salaEspera.filaEspera.isEmpty()) {
            ...
            try {
                if (salaEspera.filaEspera.isEmpty()) {
                    ...
                    dormindo = true;
                    logger.fine("Barbeiro est· dormindo");
                    barbearia.pronto.await();
                } else {
                    int time = rand.nextInt(10) + 1;
                    Cliente cliente = salaEspera.filaEspera.poll();
                    logger.fine(String.format("Barbeiro est· cortando o cabelo do cliente %d. Tamanho da fila atual: %d", cliente.getId(), salaEspera.filaEspera.size()));
                    Thread.sleep(time);
